...
async def atom_unread(request: Request, token: Annotated[str, Depends(get_current_token)], reader: bool = False):
...
async def atom_unread(tag: str, request: Request, token: Annotated[str, Depends(get_current_token)], reader: bool = False):
    return AtomResponse((await retrieve_feed(
        request_url=request.url,
        request_token=token,
        tag=tag,
